    ...
    <table border="1px" width="1000px">
        ...
        <c:forEach items="${pageInfo.list}" var="item">
            <tr>
                <td>${item.bookCode}</td>
                <td>
                    <c:if test="${item.bookType==1}"><%out.print("小说");%></c:if>
                    <c:if test="${item.bookType==2}"><%out.print("文学");%></c:if>
                    <c:if test="${item.bookType==3}"><%out.print("传记");%></c:if>
                    <c:if test="${item.bookType==4}"><%out.print("艺术");%></c:if>
                    <c:if test="${item.bookType==5}"><%out.print("少儿");%></c:if>
                    <c:if test="${item.bookType==6}"><%out.print("经济");%></c:if>
                    <c:if test="${item.bookType==7}"><%out.print("管理");%></c:if>
                    <c:if test="${item.bookType==8}"><%out.print("科技");%></c:if>
                </td>
                <td>${item.bookName}</td>
                <td>${item.bookAtuthor}</td>
                <td>${item.publishPress}</td>
                <td>
                    <c:if test="${item.isBorrow==0}"><a href=""><%out.print("申请借阅");%></a></c:if>
                    <c:if test="${item.isBorrow==1}"><%out.print("已借阅");%></c:if>
                </td>
            </tr>
        </c:forEach>
